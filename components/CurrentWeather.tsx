    ...
    <div className="w-full max-w-2xl mx-auto mt-8 bg-white/20 backdrop-blur-lg border border-white/30 rounded-3xl p-8 shadow-2xl text-white overflow-hidden relative">
      ...
      <div className="relative flex flex-col md:flex-row justify-between items-center gap-8">
        <div className="text-center md:text-left">
          ...
          <p className="text-sm uppercase tracking-widest text-white/60 mb-1">{new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
          <div className="flex items-center justify-center md:justify-start">
            <span className="text-8xl font-black tracking-tighter">{Math.round(main.temp)}°</span>
            <div className="ml-4 flex flex-col items-center">
              ...
              />
              <span className="text-lg font-medium capitalize -mt-2">{current.description}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 w-full md:w-auto">
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 flex flex-col items-center border border-white/10">
            ...
            <span className="text-xl font-bold">{Math.round(main.feels_like)}°</span>
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 flex flex-col items-center border border-white/10">
